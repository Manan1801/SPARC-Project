...
# ================== Hand-prefix mapping (your reversed CSV convention) ==================
# In the CSV: 'L_' columns actually belong to the RIGHT hand; 'R_' columns belong to LEFT.
CSV_HAND_PREFIX = {'left': 'R', 'right': 'L'}
...
def parse_args():
    ...
    # Ellipsoids control:
    #   - Omitted => no ellipsoids
    #   - Present without values (`--ellipsoids`) => left+right only
    #   - With values (`--ellipsoids both` / `--ellipsoids left right`) => draw exactly those
    parser.add_argument(
        '--ellipsoids',
        nargs='*',
...
def collect_points_3d(df, landmarks, hand):
    """Collect Nx3 points for a given hand across selected landmarks, dropping NaNs."""
...
ELLIPSOID_NSTD = 2.5       # 2σ radii
ELLIPSOID_OPACITY = 0.20
ELLIPSOID_SAMPLES = 40

# --- 3D ---
def fit_cov_ellipsoid(points, n_std=ELLIPSOID_NSTD):
    """Fit covariance ellipsoid to 3D points. Returns (center, radii, rotation, eigvals) or None."""
...
def count_points_enclosed(points, center, rotation, eigvals, n_std=ELLIPSOID_NSTD):
    """Mahalanobis inclusion count for 3D ellipsoid."""
...
def add_ellipsoid_to_fig(fig, name, X, Y, Z, center, radii, enclosed_count,
    ...
    """Add 3D ellipsoid surface with constant hover (center, axes, enclosed)."""
    if X is None:
        return
...
def fit_cov_ellipse_2d(points, n_std=ELLIPSOID_NSTD):
    """Fit covariance ellipse in 2D. Returns center(2), radii(2), rotation(2x2), eigvals(2) or None."""
...
def generate_trajectory_plot(df, selected_landmarks, axes, hands, frames, out_path, overlay_modes=None):
    """
    overlay_modes: None -> no ellipses
                    []   -> left+right (when user passed `--ellipsoids` with no values)
                    list of {'left','right','both'}
    """
    fig = go.Figure()
    color = frames
    ...
    # 2D covariance ellipses when requested
    if len(axes) == 2 and overlay_modes is not None:
        ...
        # Combined only if explicitly requested AND at least one hand is present
...
def generate_trajectory_3d(df, selected_landmarks, hands, frames, out_path, overlay_modes=None):
    """
    overlay_modes: None -> no ellipsoids
                    []   -> left+right (when user passed `--ellipsoids` with no values)
                    list of {'left','right','both'}
    """
    fig = go.Figure()
    ...
    if overlay_modes is not None:
        ...
        # Combined only if explicitly requested AND any hand present
...
def save_plot(fig, out_path, landmarks, hands, axes, plot_type):
    ...
    hand_str = ''.join(['L' if h == 'left' else 'R' for h in hands])  # keep original file-naming convention
    axes_str = ''.join(axes)
...
def main():
    ...
    # Decide overlay modes for ellipsoids/ellipses
    ...
    if args.plots in ['trajectory', 'both'] and len(axes) == 2:
        ...
        # user passed --ellipsoids but not drawing 2D trajectory
        pass
